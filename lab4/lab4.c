...
cameraTarget = (vec3) {cos((float)x / 400 * M_PI * 2) * 20,
  ...
  */
  ...
  horizontalAngle = (float)x / 600 * M_PI * 2;
  verticalAngle = (float)y / 600 * M_PI * 2;

  cameraTarget = (vec3) {cos(verticalAngle) *sin(horizontalAngle),
			 sin(verticalAngle),
			 cos(verticalAngle) *cos(horizontalAngle)};

	cameraTarget = VectorAdd(cameraTarget, cameraPos);

	vec3 rightVec = { sin(horizontalAngle - M_PI),
			  0,
			  cos(horizontalAngle - M_PI)};

	cameraDirection = Normalize(VectorSub(cameraTarget, cameraPos));

	//cameraNormal = CrossProduct(rightVec, cameraDirection);
